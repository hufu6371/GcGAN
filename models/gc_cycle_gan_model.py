...
class GcCycleGANModel(BaseModel):
    def name(self):
        return 'GcCycleGANModel'
    ...
    def initialize(self, opt):
        ...
        self.netG_BA = networks.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        if self.isTrain:
            ...
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
            self.netD_gc_A = networks.define_D(opt.output_nc, opt.ndf,
                                               opt.which_model_netD,
                                               opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
        if not self.isTrain or opt.continue_train:
            ...
            if self.isTrain:
                ...
                self.load_network(self.netD_gc_A, 'D_gc_A', which_epoch)
        ...
        if self.isTrain:
            ...
            self.fake_gc_A_pool = ImagePool(opt.pool_size)
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_AB.parameters(), self.netG_BA.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
            ...
            self.optimizer_D_A = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_gc_A.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
        ...
        if self.isTrain:
            ...
            networks.print_network(self.netD_gc_A)
        print('-----------------------------------------------')
    ...
    def backward_D_A(self):
        ...
        loss_D_A = self.backward_D_basic(self.netD_A, self.real_A, fake_A, self.netD_gc_A, self.real_gc_A, fake_gc_A)
    ...
    def get_current_visuals(self):
        ...
        ret_visuals = OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('real_B', real_B), ('fake_A', fake_A)])
        return ret_visuals
    ...
    def save(self, label):
        ...
        self.save_network(self.netD_gc_A, 'D_gc_A', label, self.gpu_ids)
    ...
    def test(self):
        ...
        self.fake_A = self.netG_BA.forward(self.real_B).data
